// 逻辑坐标系的基准大小（所有计算都基于这个 900×900 空间）
// Base logical coordinate system (everything is computed in this 900×900 space)
const W = 900, H = 900;
...
// 几何数据：列宽、行高、缝隙宽高、每列/行起点坐标（全部在 900×900 逻辑空间里）
// Geometry data in the 900×900 logical space
let colW = [], rowH = [];
let gapX = [], gapY = [];
let xs = [], ys = [];
...
let bigBlocks = [];     // 第一层大块（带 mode）| first-pass blocks with mode
